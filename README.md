...
| [`F1Square/`](F1Square/) | The **genuine-proof layer** — real Lean 4 theorems (no Mathlib, no `sorry`): the function-field Hodge mechanism (`Mechanism`, `Template`), the characteristic-1 base (`CharOne`), exact Bowen–Lanford counts (`CycleCounts`), the mechanism bridge + §2.3 control (`Bridge`), the crux stated faithfully (`Crux`), the tropical κ/spectrum stack incl. the κ⊥spectrum counterexample (`Tropical/`), and the analysis substrate — exact ℚ, a reflective ℤ ring normalizer (the no-`ring` ceiling-lifter), and constructive ℝ as Bishop regular sequences (`Analysis/`). |
