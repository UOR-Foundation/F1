...
`UOR-Framework v0.5.2` — a resolved git dependency (pinned in `lake-manifest.json`, fetched to
`.lake/packages/uor/lean4/UOR/`, not checked into this repo) — is the only *registered* UOR source. It
DOES carry an infinite typed level object — `WittLevel` (`structure WittLevel where wittLength : Nat`,
"open-world: any W_n valid", `new (n : Nat) := ⟨n⟩`, so it wraps EVERY `Nat`; `W8`,`W16`,…,`W24`,…,
`W12288` are conventional NAMED constants, NOT a type invariant) whose named levels happen to coincide
numerically with the Atlas fixture constants (`W24`,`W96`,`W128`,`W12288` = stride `T·O`, `m₀`, context
`2^(O−1)`, boundary `B₀`) — plus lift-chain / tower-completeness CERTIFICATE machinery
(`LiftChainCertificate.targetLevel : Nat`, `TowerCompletenessResolver`, φ-maps/ψ-pipeline). But it
supplies NO real-valued evaluator: its only non-`Nat` data are per-individual `Float` annotations
(entropy/Landauer/pressure/grounding), not a level-indexed `Nat → Real` diagonal; the `WittLevel`
readout `bitsWidth = wittLength` is the `Nat` level index itself, not a derived real weight and not the
eigenvalue diagonal. On the wiring: the F1 ROOT (`F1Square.lean`) DOES import six UOR ontology modules —
`UOR.Structures` and `UOR.Individuals.{Op,Schema,Convergence,Division,Homology}` — but NONE of them
defines a `Nat → Real` evaluator, and F1 imports NONE of the Witt-level/lift-chain machinery
(`UOR.Enums`/`Pipeline`/`Enforcement`, where `WittLevel`/`LiftChainCertificate` live); this
`FinAtlasOperator` cone imports none of the six either. `atlasEig` is `blockEig` over `(T,O)=(3,8)`,
independent of any imported UOR evaluator. So even the registered source provides only a
Witt-level/certificate skeleton with NO derived real evaluator restricting to `atlasSeedWeight`; the
typed parent/child refinement relation whose derived real evaluator restricts to the seed and grows
cofinally is the FORMAL BLOCKER (an audit conclusion, not a Lean theorem) — it exists in neither the
local repo nor the registered source, and is not filled by invention.
