...
-- AtlasAnchorExtract (target-free): THE BOUNDED ANCHOR EXTRACTOR BY GENUINE SCALE INTEGRATION — anchorAx z (t) = (∫_{[1+2^-k,B]} (q_k·A_pole − A_tail)(x̄,t) dx)/(∫_{[1+2^-k,B]} P_k(x̄)/4 dx) with the dual density P_k/4 = a_V⁻¹ (no pointwise division; anchorDen ≥ gap_k/(4B), reciprocal at the explicit witness 3·denLo.den), EXACT on every cut analysis at every Haar coordinate (numAx_source, anchorAx_source), and THE SOURCE-METRIC BOUND anchorAx_bound: ∫₀¹ 8wr·anchor² ≤ c_k·(poleG z.pole z.pole + tailG z.tail z.tail) for EVERY carrier element, c_k = (4B/gap_k)²·gap_k·2(B·M_K+1)² explicit (Cauchy–Schwarz unit_cs_ax/anchorXInt_sq_le from innerI_cauchy_schwarz, the pointwise square estimate num_sq_le_ax, the Fubini swap fubini_ax from bern2D_general_swap_window, pole-window monotonicity intX_tail_le_pole_ax from riemannIntegralI_split_at) — a bound on the anchor by two cut energies, not a field, not a colligation bound, no sign claim
